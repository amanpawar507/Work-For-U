...
export const Projects = () => {
  ...
  useEffect(() => {
    ...
    const getFreelancerRequests = async () => {
      try {
        const { data } = await client.get(
          `http://localhost:5000/project/requests/${user._id}`
        );
        //console.log(data);
        setRequests(data);
      } catch (error) {
    ...
    const getAllProjects = async () => {
      try {
        if (!isFreelancer) {
          const { data } = await client.get(
            `http://localhost:5000/project/all/employer/${user._id}`
          );
          //console.log(data);
          setProjects(data);
        } else {
          const { data } = await client.get(
            `http://localhost:5000/project/all/freelancer/${user._id}`
          );
          setProjects(data);
        }
      } catch (error) {
  ...
  const handleCreateProject = async (details) => {
    try {
      ...
      if (tenure === 0 || pay === 0) {
        toast({
          title: "Tenure and Hourly pay should be greater than 0",
          status: "warning",
          duration: 2000,
        });
  ...
  const handleFilter = async (filterData) => {
    try {
      ...
      const { data } = await client.get(
        `http://localhost:5000/project/filter/${userType}/${user._id}/${query}/${type}`,
        { query, type }
      );
  ...
  return (
    <Box w={"100%"}>
      ...
      {isFreelancer && (
        <>
          <Heading mb="10px" fontSize={"2xl"} borderBottom={"1px solid black"}>
            Requests
          </Heading>
        ...
        <GridItem colSpan={2}>
          {projects.length > 0 ? (
            ...
            <EmptyAlert
              text={
                isFreelancer
                  ? "You have no project for now."
                  : "No projects created. Create some!"
              }
            />
          )}
        </GridItem>
      ...
      <RateModal
        ...
        updateProject={(data) => updateProjectAfterRating(data)}
      />
    </Box>
  );
};
