  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const { data: skills } = await client.get(
    ...
    try {
      ...
      if (isFreelancer) {
        ...
        const { data } = await client.post(
          "http://localhost:5000/freelancer/",
          request
        );
        //console.log(data);
        toast({
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          {isFreelancer && (
            <InputComp
              name="expectedPay"
              label="Minimum pay expected /hr"
              value={details.expectedPay}
              onChange={handleChange}
              required={true}
            />
          )}
          ...
          {isFreelancer && (
            ...
            >
              <HStack flexWrap={"wrap"} maxH={"80px"} overflow={"auto"}>
                {skillList.map((i) => (
                  <Checkbox value={i._id} key={i._id}>
                    {i.name}
                  </Checkbox>
                ))}
              </HStack>
          ...
          <InputComp
            ...
            helper="atleast 6 characters long"
            required={true}
          />
          <InputComp
